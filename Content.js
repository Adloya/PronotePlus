...
setTimeout(() => {
  ...
  if (
    (localStorage.getItem("plus-theme") === null) &
      (localStorage.getItem("plus-theme") !== "dark") &&
    localStorage.getItem("plus-theme") !== "light" &&
    localStorage.getItem("plus-theme" !== "abyss")
  ) {
    // No theme has been set, we set the default one
  ...
  setInterval(function () {
    ...
    function patch() {
      ...
      if (document.getElementsByClassName("precedenteConnexion").length > 0) {
          ...
          if (localStorage.getItem("colorimetrie") === "false") {
            if (note.style.borderRadius !== "5px") {
              ...
              if (note.innerText.includes("/")) {
                const noteDonnee = parseFloat(
                  note.innerText.replace(",", ".").split("/")[0],
                  10,
                );
        ...
        let racext = document.createElement("section");
        racext.classList.add("widget", "discussions", "collapsible");
        racext.id = "id_84id_40";

        let racext_head = document.createElement("header");
        let racext_div = document.createElement("div");

        racext_head.title = "Services externes";
        racext_head.class = "clickable";
        racext_head.innerHTML =
          '<h2 class="clickable" style="cursor: default !important;"><span id="id_84_TitreText" tabindex="0" role="link" data-tooltip="default">Services Externes</span></h2>';

        racext_div.id = "id_84id41";
        racext_div.classList.add("content-container", "overflow-auto");
        racext_div.role = "auto";

        if (!document.getElementById("id_84_TitreText"))
          racext.appendChild(racext_head);

        if (!document.getElementById("id_84id41"))
          racext.appendChild(racext_div);

        let idetab = window.location.href
          .split("identifiant")[0]
          .replace("https://", "")
          .replace(".index-education.net/pronote/eleve.html?", "");

        let liens_services = {
          "E-Sidoc": "https://" + idetab + ".esidoc.fr/",
          Cyclades:
            "https://candidat.examens-concours.gouv.fr/cyccandidat/portal/login",
          Onisep: "https://onisep.fr/",
          Parcoursup: "https://parcoursup.gouv.fr/",
          "Scolarité Services": "https://teleservices.education.gouv.fr/eds/",
        };

        let logos_services = {
          "E-Sidoc":
            "https://www.sainte-therese-les-cordeliers.fr/wp-content/uploads/2022/04/esidoc-icone.png",
          Cyclades:
            "https://candidat.examens-concours.gouv.fr/cyccandidat/resources/images/favicon.ico",
          Onisep:
            "https://yt3.googleusercontent.com/xTe_JZ9MGButzL-omLCCefu1Iqj_nF0Qn6CcrdYI0z5O68M50RJT4EWdDdBjGQWiL27UFLVayA=s900-c-k-c0x00ffffff-no-rj",
          "Scolarité Services":
            "https://educonnect.education.gouv.fr/idp-static/favicons/favicon.ico",
          Parcoursup:
            "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSff_3OW-rRSMf-rNZ8wvDLBDv5m6vbfqcpsQ&s",
        };

        let exp_service = {
          "E-Sidoc": "Portail numérique des CDI",
          Cyclades: "Portail des examens et concours",
          Onisep: "Centre d'information à l'orientation",
          Parcoursup: "Plateforme d'accès à l'enseignement supérieur",
          "Scolarité Services": "Services administratifs",
        };

        let div_int = document.createElement("div");
        div_int.id = "id_84";

        let divint = document.createElement("ul");
        divint.style = "list-style-type: none !important;";
        divint.className = "liste_clickable";

        for (let key in liens_services) {
          if (liens_services.hasOwnProperty(key)) {
            if (!document.getElementById(liens_services[key])) {
              // key = nom du service
              // liens_services[key] = lien du service
              divint.innerHTML =
                divint.innerHTML +
                `<li id="IE.Identite.collection.g5_notes_0" style="list-style-type: none !important;"><a class="justify-between Color_RecapVS_Lue wrapper-link" href="${liens_services[key]}" target="_BLANK" style="text-decoration: none; z-index: 1000;">
                  <img height="50" width="50" src="${logos_services[key]}"></img>
                  <strong>${key}</strong>
                  <span class="date">${exp_service[key]}</span>
                </a></li>`;
            }
          }
        }

        div_int.appendChild(divint);

        racext_div.appendChild(div_int);

        let col3 = document.getElementById(
          "GInterface\.Instances\[2\]_colonne_0",
        );

        if (!document.getElementById("id_84id_40")) col3.appendChild(racext);
      }
